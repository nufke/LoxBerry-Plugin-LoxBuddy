...
if ($ajax == 'restart_lox2mqtt') {
  print "Restart LoxBuddy Server...";
  exec("cd $lbhomedir/bin/plugins/loxbuddy/server; npm run restart");
} elseif ($ajax == 'get_loxbuddy_server_pid') {
...
// Unknown request
  ...
  if (empty($ajax)) {
    error_log("ajax-loxbuddy-handler.php: ERRROR: ajax not set.");
  } else {
    error_log("ajax-loxbuddy-handler: ERRROR: ajax=$ajax is unknown.");
  }
}
