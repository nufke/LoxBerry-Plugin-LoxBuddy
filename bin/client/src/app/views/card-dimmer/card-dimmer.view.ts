...
export class CardDimmerView
  ...
  private updateVM(control: Control, subControl: SubControl): DimmerVM {
    ...
    const vm: DimmerVM = {
      ...
      ui: {
        name: (this.view === View.DETAILED) ? ((control.states.circuitNames && control.states.circuitNames[subControl.uuid]) ? control.states.circuitNames[subControl.uuid] : subControl.name) : this.translate.instant('Brightness'),
